...
//    }
     ...
     * @throws \Exception
     */
    public static function tagImage(string $pathImage, string $pathTag, string $position, $inScale, string $pathDest)
    {
        $imDest = UtilImage::loadImage($pathImage);
        $imTag = UtilImage::loadImage($pathTag);

        self::tagImage2018($imDest, $imTag, $position, $inScale, 0);
        UtilImage::saveImage($imDest, $pathDest);
    }


    /**
     * 05/2018 this version uses gd resources
     *
     * @param $imDest
     * @param $imTag
     * @param $position
     * @param $inScale
     * @throws \Exception
     * @throws \WhyooOs\Util\AssertException
     */
    public static function tagImage2018($imDest, $imTag, $position, $inScale, $paddingPercent)
    {

        // 0) check if we know the position parameter
        UtilAssert::assertInArray($position, self::ALLOWED_POSITIONS);

        // 0.5) get image sizes
        $widthSrc = imagesx($imDest);
        $heightSrc = imagesy($imDest);
        $widthTag = imagesx($imTag);
        $heightTag = imagesy($imTag);




        // 0.6) paddingInPercent --> paddingPx
        $referenceForRelativeSize = min($widthSrc, $heightSrc);
        $paddingPx = $referenceForRelativeSize * $paddingPercent / 100.0;

        //UtilDebug::dd('jksdfhjkshfjksdhfkahsfkj');
